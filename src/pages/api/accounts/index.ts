      ...
      switch (req.method) {
        case "GET":
          const getItems: AxiosResponse = await apiConnection(req, res).get(
            `/accounts?skip=${req.query.skip ?? 0}&limit=${req.query.limit ?? 50}`,
          );
          makeNextApiResponse(res, getItems);
          break;

        case "POST":
          const createItem: AxiosResponse = await apiConnection(req, res).post(
            "/accounts",
            req.body,
          );
          makeNextApiResponse(res, createItem);
          break;
